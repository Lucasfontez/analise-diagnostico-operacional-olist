...
São os passos da minha investigação pelas consultas que começa em uma tese simples e termina numa
explicação estrutural. Este documento conta essa jornada de cada query
respondendo uma pergunta para partir para a próxima. (O código e os comentários de cada uma
...
A primeira pergunta era a tese: O atraso na entrega prejudica a satisfação? Cruzando
o atraso de cada pedido (entrega real − data prometida) com a nota do review,
agrupado em faixas, a resposta veio limpa, onde a nota cai de forma contínua, de **4,29**
(no prazo) até **1,7** (atraso grave, +7 dias).

O detalhe mais revelador não é o fundo do poço, é o começo! Bastam apenas **1 a 3 dias** de
atraso para a nota perder um ponto inteiro (4,29 → **3,29**). Ou seja, não existe um
"atraso pequeno e inofensivo", pontualidade não é diferencial, é piso. Tese
confirmada. Mas somente confirmar não basta. Por isso busquei o onde, e por quê?
...
**3,84** em MA/AL), mas a query me retornou algo mais importante que o ranking: Um
**problema de método**. Estados de pouquíssimo volume como, RR com 41 pedidos, AP com 66,
AC com 80 — apareciam lado a lado com SP (40 mil). Com tão poucos clientes, a nota
média desses estados é estatisticamente frágil. Onde meia dúzia de reviews ruins muda a
média inteira.

Conclusão: Este recorte **não serve como conclusão final**. Ele serviu pra
algo melhor, me mostrou que era preciso agregar em grupos robustos. A partir daí segui com a
dimensão de regiões.

## 3. As piores notas entregam adiantado

**Query:** `atraso_por_regiao.sql`

Agregando as 27 UFs em 5 regiões, a amostra ficou mais limpa e sólida, e apareceu o ponto mais
...
Isso quebra a leitura da tese, onde se essas regiões cumprem o prazo com folga e
mesmo assim insatisfazem, **não é o atraso que derruba a nota delas**. Então o que é?
A query não responde, ela levanta a pergunta certa, onde duas hipóteses ficaram na mesa:
o tempo total de espera (e não o atraso relativo ao prazo) e o peso do frete.

## 4. Não é o atraso, é o tempo absoluto!

**Query:** `tempo_entrega_por_regiao.sql`

Aqui a medida muda, em vez de comparar a entrega com o prazo prometido, calculei o
tempo **real vivido pelo cliente** da compra até a sua entrega, e o mistério se resolve.
A nota acompanha o tempo de espera quase perfeitamente. Onde o Norte espera **22,5 dias**
(nota 4,03), Nordeste **19,9** (3,97), contra **10,7** do Sudeste (4,18). Mais que o
dobro de espera.

Insight: "entregar antes do prazo" engana. A Olist dá um prazo **folgado** ao
...
## 5. Fechamento: O frete confirma a raiz estrutural

**Query:** `frete_por_regiao.sql`

Busquei entender a segunda hipótese, que é o frete. Medindo o peso do frete sobre o preço (média
...
então o produto viaja mais, o que acaba custando mais tempo **e** mais frete ao mesmo tempo.
As duas dores somadas derrubam a satisfação. O que não é um atraso pontual a corrigir, é
...
3. Por região, surge o paradoxo de onde as piores notas entregam adiantado.
